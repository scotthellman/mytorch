...
def generate_text(
    prefix: bytes, tokenizer_fn: Path, network_fn: Path, temperature: float = 0.1
) -> bytes:
    ...
    with DisableGradManager():
        result = decoder.sample_text(prefix, 200, True, temperature=temperature)
    return result
...
def main(prefix: str, tokenizer_fn: Path, network_fn: Path, temperature: float):
    print(generate_text(prefix.encode(), tokenizer_fn, network_fn, temperature))
